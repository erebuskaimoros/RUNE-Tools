...
export async function fetchLiveRapidSwaps() {
  const [network, pools, streamingSwaps] = await Promise.all([
    thornode.getNetwork({ preferNinerealms: true }),
    thornode.getPools({ preferNinerealms: true }),
    thornode.fetch('/thorchain/swaps/streaming', { preferNinerealms: true, cache: false })
  ]);
